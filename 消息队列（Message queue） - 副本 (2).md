...
随着RabbitMQ技术本身发展，此种使用方法不再被推荐。下面也介绍一下。

首先需要创建Policies，假设命名为p2 ，之后配置下图的属性。



1. policies中添加ha-mode
...
| 使用如下队列的RabbitMQ集群 | 扩容 | 增加吞吐量 | HA   | 单点故障容错 | 多点故障容错 |
| -------------------------- | ---- | ---------- | ---- | ------------ | ------------ |
| quorum队列                 | √    | √          | √    | √            | √            |
| classic队列                | √    | √          | ×    | ×            | ×            |
| classic镜像队列            | √    | √          | 中等 | √            | 中等         |
